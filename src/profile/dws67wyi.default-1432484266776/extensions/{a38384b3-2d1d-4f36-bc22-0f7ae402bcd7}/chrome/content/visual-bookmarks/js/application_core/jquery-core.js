        ...
        return{
                    ...
                    if(app.core.dom.isObject(events) && module){
                        if (data[module]) {
                            if ('events' in data[module]) {
                                app.core.utils.extendEvents(data[module].events, events);
                            } else {
                                data[module].events = {};
                                data[module].events[Object.keys(events)] = [];
                                data[module].events[Object.keys(events)].push(events[Object.keys(events)]);
                            }
                        ...
                        for (mod in data) {
                            if (data.hasOwnProperty(mod)) {
                                ...
                                if (mod.events && mod.events[events.type]) {
                                    for (var key in mod.events[events.type]) {
                                        mod.events[events.type][key].apply(null, events.data || []);
                                    }
                                }
                            }
                        }
            ...
            storage: {
                get: function(key){
                    ...
                    var value = app.core.sqliteStorage.getItem(key);
...
//                    localStorage.setItem(key, value);
                    app.core.sqliteStorage.setItem(key, value);
                },

                remove: function (key) {
//                    localStorage.removeItem(key);
                    app.core.sqliteStorage.removeItem(key);
                },
                ...
                getBookmarks: function () {
                    ...
                    var bookmarksInStorage = app.core.sqliteStorage.getAllItems();
                    for (var key in bookmarksInStorage) {
                        if (isKeyBookmarkKey(bookmarksInStorage[key]['name'])) {
                            var bookmark = bookmarksInStorage[key]['value'];
            ...
            messages: {
                ...
                send: function(msg, val){
                    ...
                    console.log(msgObj);
                    app.core.visibleTab.start(val, function(data){
                        var options = {
                            background: data,
                            captureState: 'received',
                            ts: new Date().getTime()
                        };
                        app.core.storage.updateBookmark(val.requestGUID, options);
                    });
